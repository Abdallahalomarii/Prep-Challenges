...
const LastWord_2 = (str) => {
  // write your code here
  str = str.split(' ');
  let str2 = str.slice(str.lastIndexOf(str.indexOf()))
  str2 = str2.toString();
  return str2;
}
// -------------------------------------------------------------------------------------------------------
...
const replaceWords = (str) => {
  ...
  str.map((item, index) => {
    if (item == "I") {
      str.splice(index, 1, "We");
    }
    else if (item == "am") {
      str.splice(index, 1, "are");
    }
    else if (item == "was") {
      str.splice(index, 1, "were");
    }
  });
  return str.join(' ');
}
...
const letterCounter = (str) => {
  ...
  while (index < str.length) {
    if (char === str.charAt(index) && char !== ' ') { // add check for space character
      counter++;
    } else {
      if (char !== ' ') { // add check for space character
        newString += char + counter;
      } else {
        newString += char; // exclude whitespace from the count
      }

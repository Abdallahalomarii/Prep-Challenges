...
const employeesBonus = (arr) => {
    ...
    arr.forEach(key => {
        if (key.workHours > 8) {
            tempSalary = parseInt(key.salary);
            key.salary = tempSalary + 100;
            key.salary = key.salary.toString() + '$';
        }
        else {
            tempSalary = parseInt(key.salary);
            key.salary = tempSalary + 50;
            key.salary = key.salary.toString() + '$';
        }
    });
    return arr;
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    ...
    mouseArray.forEach(mouse => {
        keyBoardArray.forEach(keyboard => {
            let combo = mouse + keyboard;
            if (combo > output && combo <= budget) {
                output = combo;
            }
        });
    });
